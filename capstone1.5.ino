...
const char* ADMIN_HASH = "482c811da5d5b4bc6d497ffa98491e38"; // MD5 hash for "password123"
...
void setupRoutes() {
  ...
  server.on("/addSchedule", HTTP_POST, []() {
    if (checkSessionRole() != "Admin") { server.send(403, "text/plain", "Unauthorized"); return; }
  ...
  server.on("/saveNetwork", HTTP_POST, []() {
    ...
    logSecurityEvent("Admin", "UPDATED_NETWORK_CONFIG");
    server.sendHeader("Location", "/"); server.send(303);
